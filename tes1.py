...
def create_table(cursor):
    """
    Create the weather table in PostgreSQL if it doesn't exist.
    """
    create_table_query = """
...
def insert_data(cursor, df):
    """
    Insert weather data into the PostgreSQL database.
    """
    for _, row in df.iterrows():
        ...
        if station_id_result:
            ...
            cursor.execute(
                "INSERT INTO station_name (station_name) VALUES ('Station_id') RETURNING station_id;",
                (station_name_trimmed,)
            )
        ...
        # Fetch  taluk_id
        cursor.execute(
            "SELECT taluk_id FROM taluk WHERE LOWER(TRIM(taluk_name)) = LOWER(%s)",
            (taluk_name_trimmed,)
        )
        ...
        # Insert weather data
        insert_query = """
        INSERT INTO weather (_id, rainfall, station_name, latitude, longitude, district_name, taluk_name, firka_name, date)
